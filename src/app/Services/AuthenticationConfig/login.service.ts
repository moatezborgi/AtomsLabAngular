...
import {HttpClient, HttpHeaders, HttpParams} from "@angular/common/http";
 import {UserService} from "../UserManagementService/UserService/user.service";
...
export class LoginService {
 ...
 /* loadToken(){
  ...
  isInfirmier() {
    ...
    if (token) {
      const decodedToken = this.helper.decodeToken(token);
      if (decodedToken?.role === '[ROLE_INFIRMIER]') {
        return true;
      }
    }
    return false;
  ...
  isBiologist() {
    ...
    if (token) {
      const decodedToken = this.helper.decodeToken(token);
      if (decodedToken?.role === '[ROLE_BIOLOGIST]') {
        return true;
      }
    }
    return false;
  ...
  isFormateur() {
    ...
    if (token) {
      const decodedToken = this.helper.decodeToken(token);
      if (decodedToken?.role === '[ROLE_FORMATEUR]') {
        return true;
      }
    }
    return false;
  ...
  isApprentissant() {
    ...
    if (token) {
      const decodedToken = this.helper.decodeToken(token);
      if (decodedToken?.role === '[ROLE_APPRENTISSANT]') {
        return true;
      }
    }
    return false;
  ...
  isDocteur() {
    ...
    if (token) {
      const decodedToken = this.helper.decodeToken(token);
      if (decodedToken?.role === '[ROLE_DOCTEUR]') {
        return true;
      }
    }
    return false;
  ...
  isInventoryManager() {
    ...
    if (token) {
      const decodedToken = this.helper.decodeToken(token);
      if (decodedToken?.role === '[ROLE_INVENTORYMANAGER]') {
        return true;
      }
    }
    return false;
